...
const faqs = [
  { q: '作品需要先在 App Hub 上架吗？', a: '目前无需等待上架，各轮评审仍以公开源码仓库和可运行作品为准。采用 Hub 卡片包时，可按官方规范准备资料并完成本地预检；robrix2 通用商店接入与自动提交入口开放后，将另行公布使用方式。网页小程序与原生宿主扩展按各自形态提交。' },
  { q: '通过 Hub 检查，就算作品验收通过吗？', a: '不等同。Hub 主要检查应用资料、包完整性、权限与版本规则；仍需在注明版本的 OctoSense 或 robrix2 环境中演示真实任务、授权和结果核验。Agent 自动化水平及技术突破由评委依据作品和复现证据评价。' },
  { q: '分享小程序就算完成 Agentic 作品吗？', a: '分享只是入口。原生文章编辑器演示了授权、草稿和发送机制；参赛作品还要展示 Agent 如何读取获授权的状态、完成操作并核验结果。新增原生应用需提交包含宿主扩展的可运行版本。' },
  { q: '必须到 GOSIM 现场才能获奖吗？', a: '不需要。比赛在复赛结束时确定一、二、三等奖；总排名前三名将到 GOSIM 黑客松现场展示作品并参加颁奖，到场情况不影响名次与获奖资格。' },
  { q: '小程序从哪里开始？', a: '从 OctoSense-org/robrix2 小程序宿主开始。在“发现”或聊天“+”中打开文章编辑器，体验授权、草稿、原生预览、分享应用与确认发送。网页小程序也可通过 URL 卡片分享；软件工厂选题可扩展 hagency 协作。' },
  ...
  { q: '没有 Android 手机怎么办？', a: '可使用公布支持的桌面环境，不强制手机演示。当前原生文章编辑器的验证记录来自 macOS；移动端和加密会话需单独验证。OctoSense ROM 是可选技术进阶方向。' },
      ...
      <div class="hero-stage">
        <div class="hero-copy">
          ...
          <div class="hero-details">
            ...
            <p class="hero-theme">{{ t('围绕 OctoSense，在 robrix2 中创造 Agent 能理解、能操作、能完成任务的小程序。') }}</p>
    ...
    <section id="awards" class="section awards">
      ...
      <div class="awards-grid">
        <article class="champion">
          <span>{{ t('1 个获奖名额') }}</span>
          <h3>{{ t('一等奖') }}</h3>
          <p class="prize-amount">¥20,000 <span>{{ t('/ 名') }}</span></p>
          <p>{{ t('现金奖金 + MiniMax Token 额度') }}</p>
        </article>
        <article>
          <span>{{ t('2 个获奖名额') }}</span>
          <h3>{{ t('二等奖') }}</h3>
          <p class="prize-amount">¥9,000 <span>{{ t('/ 名') }}</span></p>
          <p>{{ t('现金奖金 + MiniMax Token 额度') }}</p>
        </article>
        <article>
          <span>{{ t('3 个获奖名额') }}</span>
