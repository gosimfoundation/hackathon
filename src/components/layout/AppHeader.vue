...
const modelChoices = ['Kimi', 'GLM', 'MiniMax', 'DeepSeek', 'Other']
  ...
  >
    ...
    >
      <div v-if="mobileOpen" class="lg:hidden bg-bg-primary border-t border-border px-6 py-4">
        ...
        <a href="/#teams" @click.prevent="scrollTo('#teams')" class="mt-4 block text-center px-5 py-3 bg-btn-bg text-btn-text text-xs font-semibold tracking-widest uppercase hover:bg-btn-hover transition-colors">
          {{ t('nav.applyNow') }}
        </a>
      </div>
    </Transition>
      ...
      <div v-if="showProfileModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
        ...
        <div class="relative w-full max-w-md glass-card p-8 max-h-[90vh] overflow-y-auto border-accent-blue/20">
          ...
          <div v-if="!profileEditing && user" class="space-y-4">
            ...
            <div v-if="user.lookingForTeam && !user.teamId" class="pt-3 border-t border-border-subtle text-xs text-emerald-400">Looking for a team</div>
            ...
            <div v-if="user && user.checkedIn && user.teamId" class="pt-3 border-t border-border-subtle">
              ...
              <template v-else>
                <p class="text-xs text-text-muted">Model credits are distributed by the organizers for the competition environment.</p>
              </template>
            </div>

            <div v-if="user && profileQr" class="flex flex-col items-center pt-4 mt-2 border-t border-border">
              <p class="text-xs text-text-muted uppercase tracking-wider mb-2">Your Registration QR Code</p>
              <img :src="profileQr" class="w-28 h-28" />
              <p class="text-[10px] text-text-muted mt-1">Opens your registration record</p>
            </div>
          </div>
          ...
          <div class="relative bg-bg-card border border-border w-full max-w-md max-h-[80vh] overflow-y-auto">
            ...
            <template v-if="myTeam && myTeam.leaderId === user?.id">
              <div class="p-6">
                ...
                <!-- 成员列表 -->
                <div class="mb-6">
                  <p class="text-xs text-text-muted uppercase tracking-wider mb-3 font-semibold">Members ({{ myTeam.members.length }})</p>
                  <div class="space-y-2">
                    <div v-for="m in myTeam.members" :key="m.id" class="flex items-center gap-3 p-2 bg-bg-elevated">
                      ...
                      <span v-if="m.id === myTeam.leaderId" class="text-xs text-badge-warning-text">Lead</span>
                    </div>
                  </div>
                </div>
                ...
                <div v-if="myTeam.pendingUsers?.length">
                  <p class="text-xs text-badge-warning-text uppercase tracking-wider mb-3 font-semibold">Pending Requests ({{ myTeam.pendingUsers.length }})</p>
                  <div class="space-y-2">
                    ...
                    <div v-for="t in myPendingTeams" :key="t.id" class="flex items-center justify-between gap-3 p-3 bg-bg-elevated border border-border-hover">
                      <div class="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p class="text-xs text-badge-warning-text">Pending Approval</p>
                        </div>
                      </div>
